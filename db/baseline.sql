...
--
-- Reconstructed from the live database by introspecting the catalogue,
-- because the schema predates this folder and was applied by hand. It is a
-- snapshot to read and to rebuild from, not a migration to re-run against
...
CASE
    WHEN (qty <> (0)::numeric) THEN (total_cost / qty)
    ELSE NULL::numeric
END) stored
);
...
CREATE INDEX production_runs_product_idx ON public.production_runs USING btree (product_id, occurred_at DESC);
...
CREATE UNIQUE INDEX products_active_name_norm_unique_idx ON public.products USING btree (lower(btrim(regexp_replace(name, '\s+'::text, ' '::text, 'g'::text)))) WHERE is_active;
...
create view item_stock as  WITH last_batch AS (
         ...
         SELECT m_1.item_id,
            sum(m_1.total_cost) AS purchase_spend,
            sum(m_1.qty) AS purchased_qty
           FROM stock_movements m_1
          WHERE (m_1.kind = ANY (ARRAY['receipt'::stock_movement_kind, 'production'::stock_movement_kind])) AND m_1.total_cost IS NOT NULL
          GROUP BY m_1.item_id
        )
...
CREATE OR REPLACE FUNCTION public.audit_shorten(value jsonb)
 RETURNS jsonb
 LANGUAGE sql
 IMMUTABLE
AS $function$
  /* An image is a base64 blob of a few hundred KB. Recording that it changed
     is the point; carrying both copies into the log is not. */
  select case
...
begin
  ...
  if new.raw_app_meta_data->>'provider' = 'google' then
    ...
    -- A walk-in customer the admin entered manually, by the same email,
    -- with no login yet — link it to this account rather than creating a
    -- second row for the same person (and rather than erroring on the
    -- unique email index).
...
CREATE OR REPLACE FUNCTION public.pack_sizes_audit()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
declare

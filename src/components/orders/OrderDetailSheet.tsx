      ...
      <SheetContent className="overflow-y-auto w-full sm:max-w-xl md:max-w-2xl p-6 bg-white border-l border-slate-200/80 shadow-2xl transition-all duration-300 ease-out">
        ...
        <div className="motion-safe:animate-in motion-safe:fade-in-50 motion-safe:zoom-in-95 duration-300 ease-out">
          ...
          <SheetHeader className="pb-4 border-b border-slate-100 p-0">
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge
                variant="outline"
                className={cn(
                  'px-3 py-1 text-xs font-bold rounded-full border shadow-2xs transition-all duration-300',
            ...
            <div className="flex items-center gap-1.5 pt-3">
              <Button
                ...
                size="sm"
                onClick={handlePrint}
                className="rounded-xl border-slate-200 bg-slate-50 hover:bg-orange-50 hover:border-orange-300 text-slate-700 text-xs font-semibold gap-1 py-1 px-2.5 h-8 transition-all duration-200 active:scale-95"
              >
              ...
              <div className="ml-auto min-w-[120px]">
                <Select value={order.status} onValueChange={(v: OrderStatus) => onStatusChange(order.id, v)}>
                  <SelectTrigger className="rounded-xl border-orange-200 bg-orange-50/60 font-bold text-orange-700 text-xs h-8 focus:ring-2 focus:ring-orange-500/20 transition-all">
                    <SelectValue />
                  </SelectTrigger>
          ...
          <div className="flex flex-col gap-6 py-5">
            ...
            {!isCancelled ? (
              <div className="rounded-2xl bg-slate-50/80 p-3.5 border border-slate-100/90 shadow-2xs">
                    ...
                    return (
                      <li key={step} className="flex flex-1 flex-col items-center gap-1 text-center">
                        <div className="flex w-full items-center">
                          ...
                          <span
                            className={cn(
                              'flex size-5.5 shrink-0 items-center justify-center rounded-full border-2 text-[9px] font-bold transition-all duration-300',
                              isDone ? 'border-orange-500 bg-orange-500 text-white scale-110' : 'border-slate-300 bg-white text-slate-400',
                            )}
                          >
            ...
            <div className="space-y-3">
                ...
                {order.items.map((item) => (
                  ...
                  >
                    ...
                    <img
                      ...
                      alt={item.name}
                      className="size-12 rounded-xl object-cover border border-slate-200 shadow-2xs shrink-0 transition-transform duration-300 hover:scale-105"
                    />
              ...
              {/* Total Amount Card */}
              <div className="flex items-center justify-between rounded-2xl bg-gradient-to-r from-orange-500 to-rose-500 p-4 text-white shadow-md shadow-orange-500/20 transition-all duration-300 hover:scale-[1.01]">
